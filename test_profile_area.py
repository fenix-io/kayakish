...
def test_area_calculation():
    ...
    assert np.isclose(area, expected_area, rtol=1e-10), f"Área esperada {expected_area}, obtenida {area}"
...
def test_area_triangle():
    ...
    assert np.isclose(cy, expected_cy, rtol=1e-10), f"Centroide Y esperado {expected_cy}, obtenido {cy}"
    assert np.isclose(cz, expected_cz, rtol=1e-10), f"Centroide Z esperado {expected_cz}, obtenido {cz}"
